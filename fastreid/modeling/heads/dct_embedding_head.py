...
class DCTEmbeddingHead(nn.Module):
    def __init__(self, cfg):
        ...
        self.attention = cfg.MODEL.HEADS.DCT_ATTENTION
        ...
        if self.attention:
            self.dct_fc = nn.Sequential(nn.Linear(feat_dim, feat_dim, bias=False),
    ...
    def forward(self, features, targets=None):
        ...
        global_feat = global_feat.view(global_feat.size()[:2])
        global_feat = torch_dct.dct1(global_feat)
        if self.attention:
            att_global_feat = self.dct_fc(global_feat)
            global_feat = global_feat * att_global_feat
        global_feat = global_feat.view(*global_feat.size(), 1, 1)
