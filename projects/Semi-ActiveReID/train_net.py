...
@function: Implementation of Semi-supervised Active learning for ReID
"""
...
class SemiActiveTrainer(DefaultTrainer):
    def __init__(self, cfg):
        super(SemiActiveTrainer, self).__init__(cfg)
    ...
    def run_step(self) -> None:
        assert self.model.training, "[SemiActiveTrainer] model was changed to eval mode!"
...
def main(args):
    ...
    if args.eval_only:
        ...
        model = SemiActiveTrainer.build_model(cfg)
        Checkpointer(model).load(cfg.MODEL.WEIGHTS)  # load trained model
        model = SemiActiveTrainer.build_parallel_model(cfg, model)  # parallel
        
        res = SemiActiveTrainer.test(cfg, model)
        return res
