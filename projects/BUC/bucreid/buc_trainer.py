...
class BUCTrainer(DefaultTrainer):
    def __init__(self, cfg: CfgNode) -> None:
        ...
        data_loader, self.train_set = self.build_buc_train_loader()
    ...
    def build_buc_train_loader(self, datalist=None):
        if datalist:
            data_set = datalist
        else:
    ...
    def build_transforms(self, cfg, is_train=True):
        normalizer = T.Normalize(mean=[0.485, 0.456, 0.406],
                                 std=[0.229, 0.224, 0.225])
        h, w = cfg.INPUT.SIZE_TRAIN
        if is_train:
            transformer = T.Compose([
                T.RandomSizedRectCrop(h, w),
