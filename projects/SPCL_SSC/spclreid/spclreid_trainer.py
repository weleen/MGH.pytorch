...
class SPCLTrainer(DefaultTrainer):
    ...
    def construct_unsupervised_dataloader(self,
        ...
        data_set = CommDataset(img_items, transforms, relabel=True, cfg=self.cfg, is_train=is_train)
    ...
    def run_step(self) -> None:
        ...
        if hasattr(self.model, 'module'):
            loss_dict = self.model.module.losses(outputs, self.memory, iters=self.iter)
        else:
            loss_dict = self.model.losses(outputs, self.memory, iters=self.iter)
